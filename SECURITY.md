...
This policy covers the `lexicon` application code, its Docker images, and its
GitHub Actions workflows. It does not cover third-party dependencies (report
those upstream) or self-hosted deployments the maintainer does not operate.

## Security design references

- Threat model: `docs/project-memory/06-security-threat-model.md` (not yet
  produced)
- Prompt-injection defence test suite: to be added under `docs/security/` as
  part of the Verification & Testing deep phase (see
